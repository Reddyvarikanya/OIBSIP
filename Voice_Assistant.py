...
==============================
A beginner-friendly voice assistant that:
  1. Listens to your microphone
  2. Converts speech to text (speech recognition)
  3. Understands a few basic commands
  4. Responds out loud (text to speech)

HOW IT WORKS (big picture)
---------------------------
- `speech_recognition` captures audio from your mic and sends it to Google's
  free speech-to-text API to get back text.
- `pyttsx3` converts text back into spoken audio (works offline).
- We check the recognized text for keywords ("time", "wikipedia", "open
  youtube", etc.) and run the matching function.

INSTALL (run these in your terminal first)
--------------------------------------------
    pip install SpeechRecognition pyttsx3 pyaudio wikipedia

Notes on pyaudio (the trickiest install):
  - Windows: `pip install pyaudio` usually just works.
  - Mac:     `brew install portaudio` then `pip install pyaudio`
  - Linux:   `sudo apt-get install python3-pyaudio` (or portaudio19-dev
...
Then just talk! Try saying: "what time is it", "open youtube",
"search wikipedia for python", "tell me a joke", or "stop"/"exit" to quit.
"""
