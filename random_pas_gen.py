...
def get_length():
    while True:
        ...
        print(f"[DEBUG] You typed: {raw!r}")  # shows exactly what was received

        # Strip whitespace and any stray quote characters
        cleaned = raw.strip().strip('"').strip("'")

        # Extract digits only, in case of accidental extra characters
        digits_only = "".join(ch for ch in cleaned if ch.isdigit())

        if not digits_only:
            print("Please enter a valid whole number (e.g. 12). "
                  "Don't type a password here — just the number of characters you want.\n")
            continue
...
def get_character_types():
    ...
    print("  3. Numbers (0-9)")
    print("  4. Symbols (!@#$%^&* etc.)")
    print("Type the NUMBERS of the options you want (not a password).")

    while True:
        raw = input("Your choice (e.g. 1,2,3 or 1 2 3): ")
        print(f"[DEBUG] You typed: {raw!r}")  # shows exactly what was received
        raw = raw.strip()
        ...
        if invalid_parts:
            print(
                f"'{raw}' isn't valid input. Only enter the numbers 1, 2, 3, or 4 "
                f"(separated by commas or spaces) — not a password or other text.\n"
            )
            continue
